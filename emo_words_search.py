...
class EmoWordsBase(ABC):
    """Base class for emotional words management. 
    Provides methods to get, add, remove, stem, and lemmatize emotional words."""
    ...
    def __init__(self):
        ...
        self.dict_of_emotional_words = {
            "happiness": self.happy_words,
    ...
    def get_emotional_words(self, emotion):
        """Returns the list of emotional words for a given emotion. 
        If the emotion is not found, returns an empty list.
        Args:
    ...
    def add_emotional_words(self, emotion, words):
        """Adds new emotional words to the list of emotional words for a given emotion. 
        If the emotion does not exist, it creates a new entry for it.
        Args:
    ...
    def remove_emotional_word(self, emotion, word):
        """Removes an emotional word from the list of emotional words for a given emotion. 
        If the emotion or the word is not found, it does nothing.
        Args:
        ...
        Returns:
            None: the method modifies the existing attribute dict_of_emotional_words."""
        if emotion in self.dict_of_emotional_words:
            self.dict_of_emotional_words[emotion] = [
                w for w in self.dict_of_emotional_words[emotion] if w != word
            ]

    def stem_emotional_words(self, stemmer=nltk.stem.SnowballStemmer("english")):
        """Stems the emotional words using the provided stemmer. 
        If no stemmer is provided, it uses the SnowballStemmer for English from NLTK.
        Args:
        ...
        for key in self.dict_of_emotional_words.keys():
            self.dict_of_emotional_stems[key] = [
                self.stemmer.stem(word) for word in self.dict_of_emotional_words[key]
            ]

    def lemmatize_emotional_words(self, lemmatizer=nltk.stem.WordNetLemmatizer()):
        """Lemmatizes the emotional words using the provided lemmatizer. 
        If no lemmatizer is provided, it uses the WordNetLemmatizer from NLTK.
        Args:
        ...
        for key in self.dict_of_emotional_words.keys():
            self.dict_of_emotional_lemmas[key] = [
                self.lemmatizer.lemmatize(word)
                for word in self.dict_of_emotional_words[key]
            ]
...
class EmoWordsFinder(EmoWordsBase):
    ...
    def __init__(self, generate_test, loadpath, sentences=None):
        """Initializes the EmoWordsFinder class. 
        If sentences are provided, it uses them. 
        If generate_test is True, it generates a test portfolio using the generate_test_portfolio method. 
        If loadpath is provided, it loads the sentences from the specified file (PDF or TXT).
        Args:
    ...
    def load_pdf(self, loadpath):
        """Loads sentences from a PDF file. 
        It reads the PDF file, extracts the text from each page, and splits it into sentences based on periods.
        Args:
    ...
    def load_txt(self, loadpath):
        """Loads sentences from a TXT file. 
        It reads the TXT file, extracts the text, and splits it into sentences based on periods.
        Args:
    ...
    def find_emotional_words_by_stem(self, method="AI"):
        """Finds emotional words in the sentences by stemming the words and comparing them to the stemmed emotional words in the dictionary. 
        It can use either AI or manual validation to determine if a word is used emotionally in a sentence.
        Args:
                ...
                for word in sentence.split():
                    for key in self.dict_of_emotional_stems.keys():
                        ...
                        if (
                            ...
                            in self.dict_of_emotional_stems[key]
                        ):
                            try:
    ...
    def find_emotional_words_by_exact_match(self, method="AI"):
        """Finds emotional words in the sentences by exact match of the words to the emotional words in the dictionary. 
        It can use either AI or manual validation to determine if a word is used emotionally in a sentence. 
        When using this method, it's recommended to first update the dictionary_of_emotional_words attribute with the add_emotional_words method to include different forms of the emotional words of interest (e.g., struggle, struggles, struggling) to increase the chances of finding emotional words in the sentences.
        Args:
            method (str): The method to use for validation ('AI' or 'manual'). Default: 'AI': uses the Gemini 3.1 Flash Lite model to validate if the word is used emotionally in the sentence. 'manual': prompts the user to validate if the word is used emotionally in the sentence.
        Returns:
            None: The method updates the search_results_df attribute with the findings. This DataFrame contains the columns: 'Emotion', 'Word', 'Sentence', 'Sentence_id', 'Search_method', and 'Validation_method'.
                ...
                for word in sentence.split():
                    for key in self.dict_of_emotional_words.keys():
                        ...
                        if word.strip().lower() in [
                            w.lower() for w in self.dict_of_emotional_words[key]
                        ]:
                            try:
                                response = self.client.models.generate_content(
                                    model="gemini-3.1-flash-lite",
    ...
    def find_emotional_words_by_lemma(self, method="AI"):
        """Finds emotional words in the sentences by lemmatizing the words and comparing them to the lemmatized emotional words in the dictionary. 
        It can use either AI or manual validation to determine if a word is used emotionally in a sentence. 
        When using this method, it's recommended to first update the dictionary_of_emotional_words attribute with the add_emotional_words method, as current dictionary may not be sufficient for lemmatized search.
        Args:
            method (str): The method to use for validation ('AI' or 'manual'). Default: 'AI': uses the Gemini 3.1 Flash Lite model to validate if the word is used emotionally in the sentence. 'manual': prompts the user to validate if the word is used emotionally in the sentence.
        Returns:
            None: The method updates the search_results_df attribute with the findings. This DataFrame contains the columns: 'Emotion', 'Word', 'Sentence', 'Sentence_id', 'Search_method', and 'Validation_method'.
            ...
            for j, sentence in enumerate(self.sentences):
                ...
                for word in sentence.split():
                    for key in self.dict_of_emotional_lemmas.keys():
                        ...
                        if (
                            self.lemmatizer.lemmatize(word.strip())
                            in self.dict_of_emotional_lemmas[key]
                        ):
                            try:
                                response = self.client.models.generate_content(
                                    model="gemini-3.1-flash-lite",
    ...
    def generate_test_portfolio(self, n_sentences=1):
        """Generates a test portfolio with sentences that express different emotions using the emotional words from the dictionary. 
        For each emotion, it generates n_sentences sentences that express the emotion and n_sentences sentences that use the emotional words but do not express the emotion. 
        It uses the Gemini 3.1 Flash Lite model to generate the sentences.
        Args:
